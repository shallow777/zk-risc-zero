    ...
    >,
{
    fn eval_check(
        ...
        let poly_fp_time = start_poly_fp.elapsed();
        println!("poly_fp time: {:?}", poly_fp_time);
    }
...
pub fn segment_prover(hashfn: &str) -> Result<Box<dyn SegmentProver>> {
    ...
    println!("Creating CPU segment prover with hashfn: {hashfn}");
